...
def render_descriptive_page():
    ...
    hypertension_df = (
        df.groupby("hypertension", as_index=False)["stroke"].mean().replace({"hypertension": {0: "não", 1: "sim"}})
    )
    heart_df = (
        df.groupby("heart_disease", as_index=False)["stroke"].mean().replace({"heart_disease": {0: "não", 1: "sim"}})
    )
    smoking_order = ["formerly smoked", "smokes", "never smoked", "Unknown"]
    smoking_df = (
        df.groupby("smoking_status", as_index=False)["stroke"].mean().rename(columns={"stroke": "Média de AVC"})
    )
    smoking_df["smoking_status"] = pd.Categorical(smoking_df["smoking_status"], categories=smoking_order, ordered=True)
    smoking_df = smoking_df.sort_values("smoking_status")
    ...
    with row2_col1:
        ...
        fig_hyper.update_traces(texttemplate="%{y:.4f}", hovertemplate="Hipertensão=%{x}<br>Média AVC=%{y:.4f}<extra></extra>")
        fig_hyper.update_layout(height=320, showlegend=False, xaxis_title="", yaxis_title="Média de AVC")
        ...
        fig_heart.update_traces(texttemplate="%{y:.4f}", hovertemplate="Doença Cardíaca=%{x}<br>Média AVC=%{y:.4f}<extra></extra>")
        fig_heart.update_layout(height=320, showlegend=False, xaxis_title="", yaxis_title="Média de AVC")
    ...
    with row2_col2:
        ...
        fig_smoke.update_traces(texttemplate="%{y:.4f}", hovertemplate="%{label}<br>Média AVC=%{y:.4f}<extra></extra>")
